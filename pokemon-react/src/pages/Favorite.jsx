...
export default function Favorite() {
  ...
  const handleRemoveFavorite = (pokemonName) => {
      ...
      const newFavoritePokemon = favoritePokemon.filter(pokemon => pokemon.name !== pokemonName);
  ...
  return (
      <>
          ...
          {favoritePokemon && favoritePokemon.map((pokemon) => (
              <Card key={pokemon.id} sx={{ maxWidth: 345, margin: '0 auto' }}>
                  <CardHeader
                      title={pokemon.name}
                      ...
                      <div className='flex justify-center items-center'>  
                          <Tooltip title='Remove from favorites'>
                              <IconButton onClick={() => {
                                  ...
                                  <DeleteIcon className='favorite-icon' />
                              </IconButton>
                          </Tooltip>
                      </div>
                  </CardContent>
            ...
            <div className='absolute left-[50%] translate-x-[-50%] top-[50%] translate-y-[-50%]'>
                  <div className='flex flex-col sm:flex-row justify-center items-start'>
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                          <h3 className='text-base font-semibold leading-5 sm:leading-6 text-gray-900'>You don't have any favorite pokemon</h3>
